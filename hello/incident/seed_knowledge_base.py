...
KB_ENTRIES: list[dict] = [
    ...
    # -----------------------------------------------------------------------
    {
        ...
        "content": (
            ...
            "reason=db_statement_timeout latency=5.52'.\n"
            "Breadcrumbs: [\"pg_sleep_executed\", \"statement_timeout\", "
            "\"db_timeout\"]\n"
            "RootCause: The endpoint executes 'SELECT pg_sleep(10)' which "
            "blocks the database connection. When the "
            "statement_timeout (5.5 s) fires, PostgreSQL cancels the query "
            "and SQLAlchemy raises OperationalError.\n"
            "Remediation: Set statement_timeout above the sleep duration "
            ...
            "timeout'} only when the timeout is intentionally reintroduced.\n"
            "Resolved: True\n"
        ),
    },
    {
    ...
    },
    {
        ...
        "content": (
            ...
            "ErrorCode: FAULT_EXTERNAL_API_LATENCY\n"
            "Symptoms: External API returned malformed payload data. "
            "Logs: 'FAULT_EXTERNAL_API_LATENCY "
            "route=/test-fault/external-api reason=wrong_data "
            "latency=0.45'. Error rate on the endpoint reached 30 %.\n"
            "Breadcrumbs: [\"external_api_call\", \"wrong_data\", "
            "\"payload_validation_failed\"]\n"
            "RootCause: The mock API fault mode returned HTTP 200 with "
            "a corrupted payload ({'value': 'forty-two'}) instead of the "
            "expected {'value': 42} shape.\n"
            "Remediation: Added strict payload validation and retried the "
            "request when the response body failed validation. Only accepted "
            "responses where value was the expected integer.\n"
            "Verification: Wrong payloads are rejected immediately and the "
            "endpoint succeeds once a valid payload is returned.\n"
            "Resolved: True\n"
        ),
    },
    {
