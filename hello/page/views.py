...
def install_global_ssm_error_handler():
    """Install a global exception handler to catch and properly handle SSM AccessDeniedException.
    
    This prevents AccessDeniedException from generating WARNING level logs across the entire application
    by ensuring they are handled consistently at DEBUG level in restricted environments.
    """
    import logging
    import sys
    from functools import wraps
    
    # Get all existing loggers and install the handler
    original_excepthook = sys.excepthook
    
    def ssm_aware_excepthook(exc_type, exc_value, exc_traceback):
        """Custom exception hook that handles SSM access denied errors gracefully."""
        if hasattr(exc_value, 'response'):
            response = getattr(exc_value, 'response', {}) or {}
            error = response.get('Error', {}) if isinstance(response, dict) else {}
            if error.get('Code') == 'AccessDeniedException' and 'ssm:' in str(exc_value):
                # This is an SSM access denied error - log at debug level instead of raising
                logger = logging.getLogger('hello.developer.views')
                logger.debug(
                    "SSM AccessDeniedException handled gracefully: %s. "
                    "This is expected in restricted environments.",
                    str(exc_value)
                )
                return
    ...
    sys.excepthook = ssm_aware_excepthook
    
    # Also patch the logging system to intercept SSM-related warnings
    class SSMLogFilter(logging.Filter):
        """Filter to convert SSM AccessDeniedException warnings to debug level."""
        
        def filter(self, record):
            # Check if this is a WARNING level log containing SSM AccessDeniedException
            if (record.levelno == logging.WARNING and 
                'AccessDeniedException' in record.getMessage() and 
                ('ssm:' in record.getMessage() or 'DeleteParameter' in record.getMessage())):
                # Convert WARNING to DEBUG for SSM access denied errors
                record.levelno = logging.DEBUG
                record.levelname = 'DEBUG'
                # Update the message to indicate this is expected behavior
                if 'Could not clear cooldown' in record.getMessage():
                    record.msg = record.msg.replace(
                        'Could not clear cooldown',
                        'Skipping cooldown clear (insufficient SSM permissions, expected in restricted environments)'
                    )
            return True
    
    # Install the filter on all relevant loggers
    for logger_name in ['hello.developer.views', 'hello.page.views', '__main__', '']:
        logger = logging.getLogger(logger_name)
        logger.addFilter(SSMLogFilter())
    
    # Also install on the root logger to catch any other cases
    root_logger = logging.getLogger()
    root_logger.addFilter(SSMLogFilter())


# Install the global handler when this module is imported
install_global_ssm_error_handler()
